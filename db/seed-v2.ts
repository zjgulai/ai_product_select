...
async function seedAmazonProducts() {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    products.push({
      ...
      price: String(price),
      monthlySales,
      monthlyRevenue: (monthlySales * price).toFixed(2),
...
async function seedTiktokCreators() {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    creators.push({
      ...
      monthlyRevenue: String(randomFloat(1000, 500000, 2)),
      videoGpm: String(randomFloat(0.5, 50, 2)),
      liveGpm: String(randomFloat(0, 30, 2)),
...
async function seedTiktokVideos(creators: typeof tiktokCreators.$inferInsert[]) {
  ...
  for (let i = 0; i < 300; i++) {
    ...
    videos.push({
      ...
      monthlyRevenue: String(randomFloat(0, views * 0.05, 2)),
...
async function seedTiktokShops() {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    shops.push({
      ...
      totalRevenue: String(revenue * randomFloat(10, 100, 2)),
      rating: String(randomFloat(3.5, 4.9, 2)),
...
async function seedKeywordMappings() {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    mappings.push({
      tiktokKeyword: tk,
      amazonKeyword: amz,
      similarityScore: String(randomFloat(0.65, 0.98, 3)),
...
async function seedProductConcepts() {
  ...
  for (const c of CONCEPTS) {
    concepts.push({
      ...
      confidence: String(randomFloat(0.7, 0.98, 3)),
  ...
  for (const concept of concepts) {
    ...
    for (let d = 0; d < 30; d++) {
      ...
      metrics.push({
        ...
        tiktokEngagementRate: String(randomFloat(1, 8, 2)),
        tiktokInfluencerCount: randomInt(5, 200),
        tiktokHashtagGrowth: String(randomFloat(-10, 50, 2)),
        amazonProductCount: randomInt(10, 200),
        amazonTotalSales: randomInt(1000, 500000),
        amazonAvgRating: String(randomFloat(3.5, 4.8, 2)),
        amazonReviewGrowth: String(randomFloat(-5, 30, 2)),
        amazonSellerCount: randomInt(5, 100),
        amazonNewProductRatio: String(randomFloat(0, 20, 2)),
        shiScore: shiBase.toFixed(2),
        cviScore: cviBase.toFixed(2),
        opportunityScore: Math.min(100, opportunity).toFixed(2),
        trendMomentum: String(momentum),
        vocGapScore: String(vocGap),
      });
    }
  }
...
async function seedUserFavorites(products: typeof amazonProducts.$inferInsert[]) {
  ...
  const types: Array<"amazon_product" | "tiktok_video" | "concept"> = ["amazon_product", "tiktok_video", "concept"];
